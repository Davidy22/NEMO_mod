...
figure('units','pixels','outerposition',[0 0 400+16 400+92]);
...
if ktf(k)==1
 ...
 axis([0 0.25 -0.05 0.05]);    
 title(['Fitting (Mean Length = ' num2str(L) ' mm)'])
...
figure('units','pixels','outerposition',[0 0 800+16 400+92]);
...
if ktf(j)==1
    ...
    p4=plot(t(j:j+1),SWL(j:j+1),'-','Color',[0.25 0.25 0.25],'LineWidth',3);
end
...
set(gca,'fontsize',12)
legend([p1,p2,p3,p4],['CM Speed (Mean = ' num2str(mean(SVC)) ' mm/s)'],['Wave Speed (Mean = ' num2str(mean(SWS)) ' mm/s)'],['Amplitude (Mean = ' num2str(mean(SA)) ' mm)'],['\lambda /L (Mean = ' num2str(mean(SWL)) ')'])
