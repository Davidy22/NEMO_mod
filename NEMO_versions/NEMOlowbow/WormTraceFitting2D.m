function WormTraceFitting2D(folderName, filename, fname, frames, fps, segments,AvgV,AvgVh,Vc)
...
L=0;
  for i = 1:length(x)-1
  L = abs(sqrt((x(i)-x(i+1))^2 + (y(i)-y(i+1))^2)) + L;
  end

   chisqr=sum((residual/std(yf2p)).^2)/(segments-6);
...
ws=Dataf2./Datam(round(offs/2):end-round(offs/2)-1,2);
...
saveas(gcf,['WavePlots_' fname],'tiff')
cd(oldfolder)
...
cd('Wave Table')  
  ...
  newfile= strcat(rfile,'_Results',rext);
  xlswrite(newfile,dr);
  
set(gcf,'PaperUnits','points','PaperPosition',[0,0,1280,720])
saveas(gcf,['WaveTable_' fname],'tiff')
cd(oldfolder)
toc
