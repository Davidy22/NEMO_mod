...
for s=start:finish
    ...
    imxy = imread(['DATA/top/3_' int2str(s) '.png']);                       %load images
    ...
    imxz3dproj = permute(imxz3dproj,[3,2,1]);                               %orient xz block
    imyz3dproj = repmat(imyz,1,1,length(imxz(1,:)));                        %form yz block
    imyz3dproj = permute(imyz3dproj,[2,3,1]);                               %orient yz block
    ...
    skel3dxyz=s3Dskeleton(im3dxyz);                                          %extract skeleton of voxel image
