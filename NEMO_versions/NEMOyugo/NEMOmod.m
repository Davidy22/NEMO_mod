function NEMOmod(start,finish,segnum,fps,pxpmm)
...
if NEMOstep==0
    clearvars -global -except NEMO* mainOutFolder       % ADDED
    PERXY=struct;
...
if NEMOstep==1       % STEP NOT NECESSARY ANYMORE
...
if NEMOstep==2
    recon_pixelsmod(start,finish,pxpmm) % changed                           %Extracts mask and skeleton
...
if NEMOstep==4
    segment_datamod(start,finish,segnum,fps,pxpmm)   % changed line 106     %Segments skeleton
                    % will instead create excel to '[mainOutFolder]\NEMO'
    NEMOstep=5;      % set global var baseFileName (since empty due to frames_bin removal)
                    % commented out global var folderName, since not used 
end


if NEMOstep==5 
    detect_stagemod(start,finish,segnum,fps)                                %Categorizes frames into phases
                        % line 174. Writing to mainOutFolder
    NEMOstep=6;
end

% step 6 not necessary (none of the folders exist
%% if NEMOstep==6
