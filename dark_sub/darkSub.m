...
cd(mainOutFolder)                       % up to if added
if ~exist('binarized_frames','dir')
    mkdir('binarized_frames')
end
cd binarized_frames

[PathName] = fullfile(mainOutFolder,'frames');
...
if select == 1; 
    ...
    if select2 == 1;
        disp('Press any key to continue to select a laser dark set.');
        pause;
        fprintf('\n')               
        ...
        warning('off','all')        % added (to suppress image size warning)
        imshow(frameD);
        warning('off','all')        % added
        caxis auto
        thresh = 2;
        threshRange = linspace(thresh-2, thresh+2, 10);
        close all

        figure('Position',[100,100,1000,500]);  % added pos and size
        subplot(2, 5, 10);
        ...
        for r = 1:10;   
            ...
            WB = 1 - WormSpecR;
            startStack(:, :, r) = WB;
            %imwrite(WB, sprintf('frame%1d.tif',i));
            subplot(2, 5, r);
            imshow(WB);
            title(strcat(num2str(r)));  % removed 'Threshhold Parameter'
        end

        best = input('Which threshold looks the best? (give number) ');
    ...
    warning('off','all')        % added (to suppress image size warning)
    imshow(image);
    warning('on','all')         % added
    thresh = input('What is that threshold value? ');

    imSize = length(image);
    
    % take method from frames_bin (NEMOlowbow) to find thresh of image
...
%     thresh = thresh*255;
    ...
    figure('Position',[100,100,1000,500]);      % added position and size
    subplot(2, 5, 10);
...
%         imwrite(WB, sprintf('frame%1d.tif',i));
        subplot(2, 5, r);
        imshow(WB);
        title(num2str(r),'FontSize',10);        % removed 'Threshold Parameter:'
    end
        
    best = input('Enter the digit of the frame that appears best: ');
    newThresh = threshRange(best);
    newThresh = newThresh/255;
    close all    
