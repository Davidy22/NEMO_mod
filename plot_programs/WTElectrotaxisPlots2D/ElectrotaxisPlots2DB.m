function ElectrotaxisPlots2DB(WormNum,N,T)
...
aviobj = VideoWriter(['2Detw' num2str(wn) 'v1b.avi']);
open(aviobj);
...
a04i=-0.5;
    ...
    F2 = @(a2,xf4p)a2(1)*sin(2*pi*((a2(2)*xf4p) + a2(3))).*exp(a2(4)*xf4p);
    a20 = [a01i a02i a03i a04i];
    [a2,resnorm,residual] = lsqcurvefit(F2,a20,xf4p,yf4p,[],[],options);     % added last 3 inputs
    
chisqr=sum((residual/std(yf4p)).^2)/(4);    

RLr(k,:)=[t(k) abs(1/a2(2)) abs(a2(2)) abs(a2(1)) chisqr a2(4)];      
...
figure('units','pixels','outerposition',[0 0 512+16 512+92]);
aviobj = VideoWriter(['2Detw' num2str(wn) 'v2b.avi']);
open(aviobj);
for j = 1:frames-1
...
title(['Damped Sine Fit - Reduced Chi Squared = ' num2str(chi)])
xlabel('time(s)')
...
figure('units','pixels','outerposition',[0 0 512+16 512+92]);
aviobj = VideoWriter(['2Detw' num2str(wn) 'v3.avi']);
open(aviobj);
for j = 1:frames-1


plot(t,SP,'r-','LineWidth',3);
   
axis([t(1) t(end) min(SP)-2 max(SP)+2]);

title(['Damping Coefficient (Mean Value = ' num2str(mean(SP)) ' mm^{-1})'])
xlabel('time(s)')
ylabel('damping coefficient (1/mm)') 
set(gca,'fontsize',13)
...
% 
% figure('units','pixels','outerposition',[0 0 512+16 512+92]);
% aviobj = VideoWriter(['2Detw' num2str(wn) 'v3b.avi']);
% open(aviobj);
% for j = 1:ff
% hold on
