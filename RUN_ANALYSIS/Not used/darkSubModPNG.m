% Create dir for binarized files under FOR_ANALYSIS (becomes active dir)
cd('C:\FOR_ANALYSIS')
mkdir('binarized_frames');
cd binarized_frames

[PathName] = 'C:\FOR_ANALYSIS\frames';
MyDirInfo = dir([PathName, '\*.png']);
dirSize = length(MyDirInfo);

select = input('Would you like to select a dark set? (1 for yes, 0 for no) ');
